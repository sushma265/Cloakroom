export default function SetupBanner() {
  return (
    <div className="mb-6 flex flex-col gap-2 rounded-lg border border-rust/40 bg-rust/10 px-5 py-4 text-sm text-creamDim sm:flex-row sm:items-center sm:justify-between">
      <p>
        <span className="font-semibold text-cream">No Hugging Face token detected.</span>{" "}
        Add a free token to <code className="rounded bg-ink px-1.5 py-0.5 font-tag text-xs">.env.local</code>{" "}
        as <code className="rounded bg-ink px-1.5 py-0.5 font-tag text-xs">HF_TOKEN</code> and restart{" "}
